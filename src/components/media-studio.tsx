...
export function MediaStudio({modality,title,subtitle,embedded=false,initialAudioMode="music",hideAudioModeTabs=false}:Props){
  ...
  return <div className={`creation-workspace ${modality}-workspace ${embedded?"embedded-workspace":""}`}>{!embedded&&<div className="workspace-heading"><div className="media-studio-heading"><span className="media-studio-mark">{studioIcon}</span><span><span className="kicker">{modality} studio</span><h1>{title}</h1>{modality==="video"&&<p>Turn your ideas into stunning videos with state-of-the-art AI models.</p>}</span></div>{modality==="video"?<div className="video-workflow-guide" aria-label="Video creation steps"><span className="active"><b>1</b><em>Set up<small>Configure your video</small></em></span><i/><span><b>2</b><em>Generate<small>AI creates your video</small></em></span><i/><span><b>3</b><em>Download<small>Preview &amp; export</small></em></span></div>:<div className="workspace-context">{projectId?"Project connected":"Personal workspace"}</div>}</div>}
    ...
    <div className="studio-layout creation-studio-layout">
      <form className="studio-panel inspector-panel" onSubmit={submit}>
        ...
        {(modality==="image"||modality==="video")&&maxReferences>0&&<div className="label video-reference-field">Starting frame <span className="control-hint">Optional image to animate</span><input className="input" type="file" accept="image/png,image/jpeg,image/webp" disabled={uploading||referenceFileIds.length>=maxReferences} onChange={e=>{const f=e.target.files?.[0];if(f)uploadReference(f);e.currentTarget.value="";}}/>{referenceFileIds.length>0&&<div className="small muted" style={{marginTop:6}}>{referenceFileIds.length} reference image{referenceFileIds.length===1?"":"s"} ready. <button type="button" className="link-button" onClick={()=>setReferenceFileIds([])}>Clear</button></div>}</div>}
      ...
      <section className={`studio-panel studio-result creation-canvas ${modality==="video"?videoStyles.root:""}`}>
        ...
        {modality==="video"&&!job&&<div className="video-inspiration"><span><b>Get inspired</b><small>Real prompt starters for your next scene</small></span><div>{videoStarters.map((starter,index)=><button type="button" key={starter.label} onClick={()=>{setPrompt(starter.prompt);document.querySelector<HTMLTextAreaElement>(".video-workspace .prompt-editor")?.focus();}}><i>{String(index+1).padStart(2,"0")}</i><strong>{starter.label}</strong><small>Use prompt</small></button>)}</div></div>}
      </section>
    </div>
  </div>;
}
