  ...
  return <div><div style={{marginBottom:20}}><div className="kicker">{modality} studio</div><h1 className="page-title">{title}</h1><p className="muted" style={{lineHeight:1.65,maxWidth:720}}>{subtitle}</p></div>
    <div className="studio-layout">
      <form className="card studio-panel" onSubmit={submit} style={{display:"grid",gap:14}}>
        <label className="label">Model<select className="select" value={modelId} onChange={e=>{setModelId(e.target.value);setConfirmed(false)}}>{models.map(m=><option value={m.id} key={m.id}>{m.name} · {m.tier}</option>)}</select></label>
        ...
        {modality==="video"&&<><label className="label">Duration{veoFixed?<div className="soft-card small" style={{padding:10}}>8 seconds · fixed by this VEO tier</div>:<select className="select" value={duration} onChange={e=>{setDuration(Number(e.target.value));setConfirmed(false)}}><option value={5}>5 seconds</option><option value={8}>8 seconds</option><option value={10}>10 seconds</option><option value={15}>15 seconds</option></select>}</label><label className="label">Resolution<div className="soft-card small" style={{padding:10}}>{veoFixed?"1080p":"720p launch-safe tier"}</div></label></>}
        {modality!=="audio"&&<label className="label">Aspect ratio<select className="select" value={aspect} onChange={e=>setAspect(e.target.value)}><option>16:9</option><option>9:16</option><option>1:1</option><option>4:3</option><option>3:4</option></select></label>}
