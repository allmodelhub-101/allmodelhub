  ...
  return <div><div style={{marginBottom:20}}><div className="kicker">{modality} studio</div><h1 className="page-title">{title}</h1><p className="muted" style={{lineHeight:1.65,maxWidth:720}}>{subtitle}</p></div>
    <div className="studio-layout">
      ...
      <section className="card studio-panel studio-result">
        ...
        {job?.status==="completed"&&<div style={{width:"100%"}}>{modality==="image"&&resultUrl?<Image src={resultUrl} alt="Generated result" width={1024} height={1024} unoptimized/>:modality==="video"&&resultUrl?<video controls src={resultUrl}/>:resultUrl?<audio controls src={resultUrl}/>:<p>Generation completed. Open the result URL from history.</p>}<div className="soft-card small" style={{padding:12,marginTop:14}}>Charged {Number(job.charged_credits||job.estimated_credits||0).toFixed(2)} Credits</div>{resultUrl&&<div className="generation-actions"><a className="btn btn-primary" href={resultUrl} download>Download</a><button className="btn" type="button" onClick={async () => { await navigator.clipboard?.writeText(resultUrl); setCopied(true); window.setTimeout(() => setCopied(false), 1800); }}>{copied ? "Copied" : "Share link"}</button><button className="btn" type="button" onClick={() => { setJob(null); setError(""); }}>Regenerate</button></div>}</div>}
      </section>
    </div>
  </div>;
}
