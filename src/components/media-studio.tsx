...
import { ArrowClockwise, DownloadSimple, LinkSimple, MagicWand, MusicNote, PaperPlaneTilt, VideoCamera } from "@phosphor-icons/react";
...
export function MediaStudio({modality,title,subtitle,embedded=false,initialAudioMode="music",hideAudioModeTabs=false}:Props){
  ...
  return <div className={`creation-workspace ${modality}-workspace ${embedded?"embedded-workspace":""}`}>{!embedded&&<div className="workspace-heading"><div className="media-studio-heading"><span className="media-studio-mark">{studioIcon}</span><span><span className="kicker">{modality} studio</span><h1>{title}</h1></span></div><div className="workspace-context">{projectId?"Project connected":"Personal workspace"}</div></div>}
