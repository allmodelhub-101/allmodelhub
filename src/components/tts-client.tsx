...
export function TtsClient() {
  ...
  return <div className="studio-layout"><form className="card studio-panel" onSubmit={submit} style={{ display: "grid", gap: 14 }}><div className="kicker">Text to speech</div><h2 style={{ margin: 0 }}>Eleven Flash Voice</h2><label className="label">Text<textarea className="textarea" value={text} onChange={(event) => { setText(event.target.value); setConfirmed(false); }} maxLength={10000} required placeholder="Write your voiceover…" /></label><label className="label">Voice<PremiumSelect value={voice} onChange={setVoice} options={[{value:"EXAVITQu4vr4xnSDxMaL",label:"Sarah · Reassuring female"},{value:"hpp4J3VqNfWAUOO0d1Us",label:"Bella · Bright female"},{value:"pNInz6obpgDQGcFmaJgB",label:"Adam · Deep male"},{value:"JBFqnCBsd6RMkjVDRZzb",label:"George · British storyteller"},{value:"onwK4e9ZLuTAKqWW03F9",label:"Daniel · Broadcaster"}]} /></label><div className="confirm-box">Estimated usage: <b>{estimate.toFixed(2)} Credits</b>. The wallet reserves usage before synthesis and captures the final charge after a successful provider response.</div>{expensive && <label className="check-row"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /> I understand this voice generation may cost about {estimate.toFixed(2)} Credits.</label>}<button className="btn btn-primary" disabled={busy || !text.trim() || (expensive && !confirmed)}>{busy ? "Generating voice…" : "Generate voice"}</button>{error && <div className="soft-card small" style={{ padding: 12, color: "var(--danger)" }}>{error}</div>}</form><section className="card studio-panel studio-result">{url ? <div><audio controls src={url} /><div className="muted small" style={{ marginTop: 12 }}>{credits ? `${Number(credits).toFixed(2)} Credits charged` : "Voice ready"}</div></div> : <div><div style={{ fontSize: 50 }}>◖))</div><h3>Your voiceover will appear here</h3></div>}</section></div>;
}
