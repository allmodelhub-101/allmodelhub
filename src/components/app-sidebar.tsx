  ...
  return <>
    <aside className={`app-sidebar ${collapsed ? "is-collapsed" : ""}`} aria-label="Workspace navigation">
      ...
      <nav className="sidebar-scroll">
        ...
        <details className="sidebar-more"><summary><DotsThree className="sidebar-icon" size={20} weight="bold" aria-hidden="true" /><span className="sidebar-label">More</span></summary><div className="sidebar-nav">{moreItems.map((item) => <NavLink item={item} pathname={pathname} collapsed={collapsed} key={item.href} />)}</div></details>
