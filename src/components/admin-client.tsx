...
export function AdminClient(props: {
  ...
  return <div style={{ display: "grid", gap: 20 }}>
    ...
    {tab === "providers" && <section><h2 className="page-title" style={{ marginBottom: 14 }}>Provider routing</h2><form className="card studio-panel" onSubmit={createProviderRoute} style={{ marginBottom: 14 }}><div className="form-grid"><label className="label">AMH model<PremiumSelect value={models[0]?.id || ""} onChange={(value) => { const input = document.querySelector('input[name="modelId"]') as HTMLInputElement | null; if (input) input.value = value; }} options={models.map((model) => ({ value: model.id, label: model.display_name }))} /></label><label className="label">Provider<PremiumSelect value="apimodels" onChange={(value) => { const input = document.querySelector('input[name="providerKey"]') as HTMLInputElement | null; if (input) input.value = value; }} options={[{ value: "apimodels", label: "APIMODELS" }, { value: "haimaker", label: "Haimaker" }]} /></label><input type="hidden" name="modelId" value={models[0]?.id || ""} /><input type="hidden" name="providerKey" value="apimodels" /><label className="label">Upstream model<input className="input" name="upstreamModel" required /></label><label className="label">Priority<input className="input" name="priority" type="number" defaultValue={100} min={1} max={1000} /></label></div><button className="btn btn-primary" type="submit">Add / update route</button></form><div className="table-wrap"><table><thead><tr><th>AMH model</th><th>Provider</th><th>Upstream model</th><th>Priority</th><th>Status</th></tr></thead><tbody>{routes.length ? routes.map((route) => <tr key={`${route.model_id}:${route.provider_key}`}><td>{route.model_id}</td><td>{route.provider_key}</td><td><input className="input" style={{ minWidth: 210, padding: 7 }} defaultValue={route.upstream_model} onBlur={(event) => saveProviderRoute(route, { upstream_model: event.target.value })} /></td><td><input className="input" style={{ width: 80, padding: 7 }} type="number" min={1} max={1000} defaultValue={route.priority} onBlur={(event) => saveProviderRoute(route, { priority: Number(event.target.value) })} /></td><td><button className={`btn ${route.active ? "btn-primary" : ""}`} onClick={() => saveProviderRoute(route, { active: !route.active })}>{route.active ? "Active" : "Disabled"}</button></td></tr>) : <tr><td colSpan={5} className="muted">No provider routes configured.</td></tr>}</tbody></table></div></section>}
