...
import { ArrowsClockwise, DownloadSimple, MagicWand, PaperPlaneTilt, Plus, ShareNetwork, SlidersHorizontal, X } from "@phosphor-icons/react";
  ...
  return <div className={`image-workspace ${dragActive ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragActive(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragActive(false); }} onDrop={(event) => { event.preventDefault(); setDragActive(false); const file = event.dataTransfer.files[0]; if (file) void uploadReference(file); }}>
    ...
    <main className="image-canvas-shell">
      <section className={`image-canvas-stage aspect-${aspect.replace(":", "-")}`} aria-live="polite">
        {!job && <div className="image-stage-identity"><span>Image Studio</span><strong><i />4K Premium · {references.length ? "Edit mode" : "Create mode"}</strong></div>}{!job && <div className="image-empty-state"><div className="image-stage-emblem"><span className="canvas-orbit" aria-hidden="true"><MagicWand weight="fill" /></span><i/><i/><i/></div><h1>Create an image</h1><p>Describe your idea, customize the look, then create.</p><div className="image-creation-path" aria-label="How image creation works"><span><b>1</b>Describe</span><i/><span><b>2</b>Customize</span><i/><span><b>3</b>Create</span></div><div className="direction-starters" aria-label="Prompt ideas">{directionStarters.map((starter) => <button type="button" key={starter.label} onClick={() => setPrompt(starter.prompt)}><span>{starter.label}</span><small>Use direction</small></button>)}</div></div>}
