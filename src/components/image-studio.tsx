...
export function ImageStudio({crossModalityHandoffs=false}:{crossModalityHandoffs?:boolean}) {
  ...
  return <div className={`image-workspace ${dragActive ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragActive(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragActive(false); }} onDrop={(event) => { event.preventDefault(); setDragActive(false); const file = event.dataTransfer.files[0]; if (file) void uploadReference(file); }}>
    ...
    <header className="image-workspace-bar"><div className="image-workspace-title"><span className="image-studio-mark"><ImageSquare weight="duotone" /></span><span><strong>Images</strong><span className="image-mode-chip">{references.length ? "Edit mode" : "Create mode"}</span></span></div><nav><Link href="/history">My images</Link><button type="button" className={inspectorOpen ? "active" : ""} onClick={() => setInspectorOpen((value) => !value)} aria-expanded={inspectorOpen}><SlidersHorizontal />Controls</button></nav></header>
    <main className="image-canvas-shell">
      <section className={`image-canvas-stage aspect-${aspect.replace(":", "-")}`} aria-live="polite">
        {!job && <div className="image-empty-state"><div className="image-stage-emblem"><span className="canvas-orbit" aria-hidden="true"><MagicWand weight="fill" /></span><i/><i/><i/></div><h1>Create an image</h1><p>Describe what you imagine, or begin with a direction.</p><div className="direction-starters" aria-label="Prompt ideas">{directionStarters.map((starter) => <button type="button" key={starter.label} onClick={() => setPrompt(starter.prompt)}><span>{starter.label}</span><small>Try this look</small></button>)}</div></div>}
    ...
    <div className="image-prompt-dock"><form onSubmit={submit}><textarea value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={2} placeholder={references.length ? "Describe the change you want…" : "Describe an image…"} aria-label="Image prompt" /><div className="image-prompt-footer"><div className="image-compose-tools">{supportsEditing && <button type="button" className="compose-tool" onClick={() => fileInputRef.current?.click()}><Plus aria-hidden="true" /><span>{references.length ? `${references.length} reference` : "Add image"}</span></button>}<button type="button" className="compose-tool model-tool" onClick={() => setPickerOpen(true)}><span><small>Model</small>{model?.name || "Choose model"}</span></button><button type="button" className="compose-tool" onClick={() => setInspectorOpen(true)}><SlidersHorizontal aria-hidden="true" /><span>{aspect}</span></button></div><span className="dock-estimate">{estimate ? `~${estimate.toFixed(2)} cr` : "—"}</span><button className="image-generate-button" disabled={submitting || generating || !modelId || !prompt.trim() || Boolean(references.length && !supportsEditing) || (expensive && !confirmed)}><PaperPlaneTilt aria-hidden="true" weight="fill" />{submitting ? "Submitting…" : generating ? status.step : references.length ? "Edit" : "Create"}</button></div></form>{error && <div className="image-error" role="alert"><strong>Couldn’t continue.</strong><span>{error}</span></div>}</div>
