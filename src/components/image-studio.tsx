  ...
  return <div className={`image-workspace ${dragActive ? "is-dragging" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragActive(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragActive(false); }} onDrop={(event) => { event.preventDefault(); setDragActive(false); const file = event.dataTransfer.files[0]; if (file) void uploadReference(file); }}>
    ...
    <main className="image-canvas-shell">
      <section className={`image-canvas-stage aspect-${aspect.replace(":", "-")}`} aria-live="polite">
        {!job && <div className="image-empty-state"><div className="image-stage-emblem"><span className="canvas-orbit" aria-hidden="true"><MagicWand weight="fill" /></span><i/><i/><i/></div><h1>Create an image</h1><p>Describe your idea, customize the look, then create.</p><div className="image-creation-path" aria-label="How image creation works"><span><b>1</b>Describe</span><i/><span><b>2</b>Customize</span><i/><span><b>3</b>Create</span></div><div className="direction-starters" aria-label="Prompt ideas">{directionStarters.map((starter) => <button type="button" key={starter.label} onClick={() => setPrompt(starter.prompt)}><span>{starter.label}</span><small>Use direction</small></button>)}</div></div>}
    ...
    <div className="image-prompt-dock"><form onSubmit={submit}><label className="image-prompt-label" htmlFor="image-creation-prompt"><b>Describe your image</b><span>Be specific about subject, style, lighting, and composition</span></label><textarea id="image-creation-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={2} placeholder={references.length ? "What should change in this image?" : "A cinematic product photo with soft studio lighting…"} aria-label="Image prompt" /><div className="image-prompt-footer"><div className="image-compose-tools">{supportsEditing && <button type="button" className="compose-tool" onClick={() => fileInputRef.current?.click()}><Plus aria-hidden="true" /><span><small>Reference</small>{references.length ? `${references.length} image ready` : "Add image"}</span></button>}<button type="button" className="compose-tool model-tool" onClick={() => setPickerOpen(true)}><span><small>AI model</small>{model?.name || "Choose model"}</span></button><button type="button" className="compose-tool canvas-tool" onClick={() => setInspectorOpen(true)}><SlidersHorizontal aria-hidden="true" /><span><small>Canvas</small>{aspect}</span></button><button type="button" className="compose-tool settings-tool" onClick={() => setInspectorOpen(true)}><span><small>More</small>All settings</span></button></div><span className="dock-estimate"><small>Est. cost</small>{estimate ? `~${estimate.toFixed(2)} credits` : "—"}</span><button className="image-generate-button" disabled={submitting || generating || !modelId || !prompt.trim() || Boolean(references.length && !supportsEditing) || (expensive && !confirmed)}><PaperPlaneTilt aria-hidden="true" weight="fill" /><span>{submitting ? "Submitting…" : generating ? status.step : references.length ? "Edit image" : "Create image"}</span></button></div></form>{error && <div className="image-error" role="alert"><strong>Couldn’t continue.</strong><span>{error}</span></div>}</div>
