...
type Props = { models: CatalogModel[]; fxRate: number; displayPrice: (model: CatalogModel, fxRate: number) => string };
const modalities = ["all", "text", "image", "video", "audio"] as const;
const tiers = ["all", "budget", "balanced", "premium", "flagship"] as const;

export function ModelsMarketplace({ models, fxRate, displayPrice }: Props) {
  ...
  return <div className="marketplace">
    <div className="marketplace-controls glass"><div className="filter-group"><span className="filter-label">Capability</span>{modalities.map((item) => <button key={item} className={`mode-pill ${modality === item ? "active" : ""}`} onClick={() => setModality(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div><div className="filter-group"><span className="filter-label">Tier</span>{tiers.map((item) => <button key={item} className={`mode-pill ${tier === item ? "active" : ""}`} onClick={() => setTier(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div></div>
    {modality === "all" && tier === "all" && <section className="featured-models"><div className="section-label"><span className="kicker">Curated frontier</span><h2>Featured models</h2><p className="muted">The strongest tools in the catalog, ready for ambitious work.</p></div><div className="featured-grid">{featured.map((model) => <ModelCard key={model.id} model={model} fxRate={fxRate} displayPrice={displayPrice} href={hrefFor(model)} featured />)}</div></section>}
    <div className="marketplace-heading"><div><span className="kicker">Runtime catalog</span><h2>Every model, one workspace</h2></div><span className="muted small">{filtered.length} models</span></div><div className="model-grid">{filtered.map((model) => <ModelCard key={model.id} model={model} fxRate={fxRate} displayPrice={displayPrice} href={hrefFor(model)} />)}</div>
  </div>;
}

function ModelCard({ model, fxRate, displayPrice, href, featured = false }: { model: CatalogModel; fxRate: number; displayPrice: Props["displayPrice"]; href: string; featured?: boolean }) {
  return <article className={`card model-card ${featured ? "model-card-featured" : ""}`}><div className="model-card-top"><span className={`tier tier-${model.tier}`}>{model.tier}</span><span className="category-badge">{model.modality}</span></div><div className="model-card-title"><div><h3>{model.name}</h3><p className="muted small">{model.providerFamily}</p></div><span className="model-signal" aria-label={`${model.tier} tier`} /></div><p className="model-description">{model.description}</p><div className="capability-row">{model.capabilities.slice(0, 5).map((capability) => <span className="mini-chip" key={capability}>{capability}</span>)}</div><div className="model-price"><span className="muted small">Pricing</span><strong>{displayPrice(model, fxRate)}</strong></div><Link className="btn btn-primary" href={href}>{model.modality === "text" ? "Use model" : "Open studio"}<span aria-hidden="true">↗</span></Link></article>;
}
